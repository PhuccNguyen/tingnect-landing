export interface WalletData {
  ...
  discord?: string;
  website?: string;
  nftCount: number; // Changed from nfts to nftCount
  tokens?: number;
  transactions: number;
  portfolio: number; // Added portfolio field
  created?: string;
  networks: string[]; // Changed from network to networks array
  balance?: string;
  verified: boolean;
...
export const demoWallets: WalletData[] = [
  {
    ...
    name: "Alex Chen",
    bio: "Full-stack developer passionate about DeFi and Web3 infrastructure. Building the next generation of decentralized applications.",
    ...
    verified: true,
    role: "developer",
    interests: ["DeFi", "NFTs", "Layer 2", "Smart Contracts"]
  },
  {
    address: "0xfCcf62e5a6B838689E0D21961775b80fb8645666",
    name: "Sarah Kim",
    bio: "Blockchain researcher and DeFi protocol architect. Leading innovation in cross-chain interoperability.",
    ...
    verified: true,
    role: "founder",
    interests: ["Cross-chain", "DeFi", "Research", "Protocol Design"]
  },
  {
    address: "0xB143856Ab1224D722Dbd0576E3Fc05D98f345666",
    name: "Mike Johnson",
    bio: "Web3 investor and startup advisor in Southeast Asia. Supporting the next wave of blockchain innovation.",
    ...
    verified: true,
    role: "investor",
    interests: ["GameFi", "Metaverse", "Investment", "Startups"]
  },
  {
    address: "0x0635c20a7A9EDeC21D07817B98953C7Af73dB666",
    name: "Emily Rodriguez",
    bio: "Smart contract auditor and security researcher. Ensuring the safety of DeFi protocols.",
    twitter: "@emily_security",
    ...
    verified: true,
    role: "developer",
    interests: ["Security", "Auditing", "Smart Contracts", "DeFi"]
  },
  ...
  ...Array.from({ length: 96 }, (_, i) => {
    ...
    const names = [
      ...
      "Sophie Chen", "Marcus Johnson", "Isabella Lopez", "Ryan Kim", "Olivia Martinez",
      "Daniel Wu", "Grace Anderson", "Kevin Park", "Zoe Thompson", "Adrian Chang",
      "Maya Patel", "Nathan Torres", "Chloe White", "Ethan Singh", "Ava Rodriguez",
      "Lucas Martinez", "Mia Johnson", "Jacob Lee", "Ella Davis", "Noah Wilson",
      "Lily Kim", "Owen Brown", "Zara Ahmed", "Caleb Taylor", "Nora Zhang",
      "Isaac Chen", "Ruby Garcia", "Leo Wang", "Iris Liu", "Max Rodriguez",
      "Violet Singh", "Felix Park", "Nova Martinez", "Orion Lee", "Luna Torres",
      "Atlas Johnson", "Sage Davis", "Phoenix Wu", "River Kim", "Sky Anderson"
    ];
    
    const roles: Array<'developer' | 'founder' | 'investor' | 'builder' | 'community'> = 
      ["developer", "founder", "investor", "builder", "community"];
    
    const networks = ["ethereum", "polygon", "bsc", "arbitrum"];
    
    const interestsList = [
      ["DeFi", "NFTs", "GameFi"],
      ["Layer 2", "Cross-chain", "Privacy Tech"],
      ["Metaverse", "AI + Blockchain", "DAO"],
      ["Green Blockchain", "Education", "Web3 Social"],
      ["Smart Contracts", "Security", "Protocol Design"],
    ...
    const selectedNetworks = [...networks].sort(() => 0.5 - Math.random()).slice(0, networkCount);
    
    return {
      address: addresses[i % addresses.length] || `0x${Math.random().toString(16).substr(2, 40)}`,
      name: names[i % names.length] || `User ${i + 5}`,
      bio: `Passionate ${role} building innovative solutions in the Web3 ecosystem. Focused on creating value and driving adoption.`,
      twitter: `@${names[i % names.length]?.toLowerCase().replace(' ', '') || `user${i + 5}`}`,
      nftCount: Math.floor(Math.random() * 500) + 1,
      tokens: Math.floor(Math.random() * 200) + 5,
      transactions: Math.floor(Math.random() * 5000) + 100,
      portfolio: Math.floor(Math.random() * 1000000) + 10000,
...
export function searchWallets(query: string): WalletData[] {
  ...
  return demoWallets.filter(wallet => 
    ...
    wallet.role.toLowerCase().includes(searchTerm) ||
    wallet.interests?.some(interest => interest.toLowerCase().includes(searchTerm))
  );
}
